      ...
      <main ref={scrollRef} id="main-scroll-container" className="flex-1 relative z-10 ios-scroll overflow-y-auto overflow-x-hidden bg-slate-50" style={{ paddingBottom: `calc(${mainPaddingBottom}px + env(safe-area-inset-bottom))` }}>
        <Suspense fallback={
          ...
          <Routes>
            ...
            <Route path="/themes/:themeName/*" element={<PageWrapper><ThemeDetail onToggleOwned={onToggleOwned} onBulkToggleOwned={onBulkToggleOwned} allMinifigs={allMinifigs} onShowSubCatModal={(isOpen: boolean) => updateModalState('themeDetailSubCat', isOpen)} dataLoading={dataLoading} user={user} /></PageWrapper>} />
            ...
            <Route path="/lego-star-wars-minifigure-archive/army-builders/*" element={<PageWrapper><ArmyBuilders allMinifigs={allMinifigs} onToggleOwned={onToggleOwned} /></PageWrapper>} />
            <Route path="/lego-star-wars-minifigure-archive/character-centerpieces/*" element={<PageWrapper><CharacterCenterpieces allMinifigs={allMinifigs} onToggleOwned={onToggleOwned} /></PageWrapper>} />
            <Route path="/lego-star-wars-minifigure-archive/elite-specialists/*" element={<PageWrapper><EliteSpecialists allMinifigs={allMinifigs} onToggleOwned={onToggleOwned} /></PageWrapper>} />
            <Route path="/lego-star-wars-minifigure-archive/support-units/*" element={<PageWrapper><SupportUnits allMinifigs={allMinifigs} onToggleOwned={onToggleOwned} /></PageWrapper>} />
            <Route path="/search/*" element={<PageWrapper><SearchResults allMinifigs={allMinifigs} onToggleOwned={onToggleOwned} onBulkToggleOwned={onBulkToggleOwned} dataLoading={dataLoading} user={user} /></PageWrapper>} />
...
const GlobalHeader = ({ user, onLogout, hasActiveModal }: { user: UserProfile | null; onLogout: () => void; hasActiveModal: boolean }) => {
  ...
  const getPageTitle = () => {
    ...
    if (path === '/lego-star-wars-minifigure-archive') return 'COLLECTOR ARCHIVE';
    if (path === '/lego-star-wars-minifigure-archive/army-builders') return 'ARMY BUILDERS';
    if (path === '/lego-star-wars-minifigure-archive/character-centerpieces') return 'CHARACTER CENTERPIECES';
    if (path === '/lego-star-wars-minifigure-archive/elite-specialists') return 'ELITE SPECIALISTS';
    if (path === '/lego-star-wars-minifigure-archive/support-units') return 'SUPPORT UNITS';
    return '';
  };
  ...
  useEffect(() => {
    ...
    const fetchRanking = async () => {
      try {
        ...
        if (!isAbortError(err)) console.error("Collector ranking fetch error:", err);
        setCollectorRanking([]);
      }
    };
