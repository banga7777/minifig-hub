...
import MinifigFinder from './components/MinifigFinder';
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 flex flex-col">
      ...
      <main className="flex-1 relative z-10 ios-scroll" style={{ paddingBottom: `calc(${mainPaddingBottom}px + env(safe-area-inset-bottom))` }}>
        <Routes>
          ...
          <Route path="/minifig/:id" element={<MinifigDetail onToggleOwned={onToggleOwned} allMinifigs={allMinifigs} user={user} />} />
          <Route path="/theme/:themeName" element={<ThemeDetail onToggleOwned={onToggleOwned} onBulkToggleOwned={onBulkToggleOwned} allMinifigs={allMinifigs} onShowSubCatModal={(isOpen: boolean) => updateModalState('themeDetailSubCat', isOpen)} />} />
          ...
          <Route path="/scan" element={<MinifigFinder allMinifigs={allMinifigs} />} />
...
const GlobalHeader = ({ user, onLogout, hasActiveModal }: { user: UserProfile | null; onLogout: () => void; hasActiveModal: boolean }) => {
  ...
  const getPageTitle = () => {
    ...
    if (path.startsWith('/theme/')) return decodeURIComponent(path.split('/')[2]);
    if (path.startsWith('/minifig/')) return 'FIGURE DETAILS';
