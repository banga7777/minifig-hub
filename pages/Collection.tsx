      ...
      <div className="max-w-5xl mx-auto px-4 -mt-6 relative z-20">
        <div className="bg-white rounded-[1.5rem] p-2.5 shadow-xl shadow-slate-900/5 border border-slate-100 mb-6 space-y-2.5">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 px-1.5 pb-0.5">
            ...
            <select value={sortBy} onChange={(e) => setSortBy(e.target.value as SortOption)} className="appearance-none bg-white border border-slate-100 rounded-lg h-7 pl-2 pr-6 text-[8px] font-black text-slate-400 uppercase tracking-widest outline-none"><option value="value">PRICE</option><option value="newest">LATEST</option><option value="name">A-Z</option><option value="theme">THEME</option></select>
          </div>
        </div>
              ...
              (Object.entries(nestedGroupedData) as [string, Record<string, Minifigure[]>][]).map(([themeName, subGroups]) => (
                <section key={themeName} className="relative">
                  <div className="flex items-center justify-between mb-4 border-b-2 border-slate-200 pb-2 sticky top-14 bg-slate-50/95 backdrop-blur-sm z-30 pt-2 px-1">
                    ...
                    <Link to={`/themes/${encodeURIComponent(themeName.replace(/ /g, '-'))}`} className="text-[8px] font-black text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded-full uppercase tracking-widest">View Theme</Link>
                  </div>
