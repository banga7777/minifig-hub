...
const SearchResults: React.FC<SearchResultsProps> = ({ allMinifigs = [], onToggleOwned, onBulkToggleOwned }) => {
  ...
  const filteredResults = useMemo(() => {
    ...
    let resultWithScores = allMinifigs.map(m => {
      ...
      score += (terms.filter(t => mName.includes(t)).length * 500);
    ...
    return finalResult.sort((a, b) => {
      ...
      if (sortBy === 'name') comparison = decodeHTMLEntities(a.minifig.name).localeCompare(decodeHTMLEntities(b.minifig.name));
