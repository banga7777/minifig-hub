        ...
        {filteredAndSortedThemes.length > 0 ? (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3 md:gap-6">
            {filteredAndSortedThemes.map(theme => {
              ...
              return (
                                <Link key={theme.name} to={`/themes/${encodeURIComponent(theme.name.replace(/ /g, '-'))}`} className="group bg-white rounded-[1.8rem] p-2 border border-slate-100 shadow-sm transition-all duration-300 flex flex-col animate-in fade-in zoom-in-95 duration-300">
