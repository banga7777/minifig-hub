        ...
        {minifig.image_url ? (
          <img 
            ...
            onError={(e) => {
              ...
              const itemNo = minifig.item_no.toUpperCase();
              // Try standard BrickLink MN image path as fallback
              if (!target.src.includes('ItemImage/MN/0/')) {
                target.src = `https://img.bricklink.com/ItemImage/MN/0/${itemNo}.png`;
              } else if (!target.src.includes('no_image.png')) {
                target.src = 'https://www.bricklink.com/img/no_image.png';
              }
            }}
          />
        ) : (
        ...
        <div className={`absolute top-0 right-0 flex flex-col items-end z-20 ${isSelectionMode ? 'hidden' : ''}`}>
          ...
          >
            <i className={`${
              minifig.owned 
                ? 'fas text-rose-500' 
                : isDark ? 'far text-white/20 hover:text-rose-400' : 'far text-slate-300 hover:text-rose-300'
              } fa-heart text-[14px] transition-colors`}
            ></i>
          </button>
        </div>
