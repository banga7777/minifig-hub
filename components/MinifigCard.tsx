...
const MinifigCard: React.FC<MinifigCardProps> = ({ minifig, onToggleOwned, onClick, isSelected, isSelectionMode }) => {
  ...
  const changePercent = minifig.change_percent;

  return (
    <div 
      className={`bg-white rounded-xl p-1.5 shadow-sm border group transition-all duration-200 relative ${isSelected ? 'border-indigo-500 ring-2 ring-indigo-500' : 'border-slate-100'} ${isSelectionMode ? 'cursor-pointer' : ''} ${isSelectionMode && !isSelected ? 'opacity-70' : ''}`}
      onClick={onClick}
    ...
    >
      ...
      <div 
        className="aspect-square rounded-lg flex items-center justify-center overflow-hidden relative img-container-skeleton"
      >
        {minifig.image_url ? (
          <img 
            ...
            alt={decodedName} 
            loading="lazy"
            className="w-[92%] h-[92%] object-contain p-0.5 relative z-10"
          />
        ) : (
        ...
        {priceDisplay && !isSelectionMode && (
          <div className="absolute top-1.5 left-1.5 z-20 flex flex-col gap-1 items-start">
            ...
            {changePercent !== undefined && changePercent !== 0 && (
              <div className={`text-[7px] font-black px-1 py-0.5 rounded shadow-sm backdrop-blur-sm border ${changePercent >= 0 ? 'bg-emerald-500/90 text-white border-emerald-400/50' : 'bg-rose-500/90 text-white border-rose-400/50'}`}>
                <span className="hidden bg-emerald-500/90 text-white border-emerald-400/50"></span>
                <span className="hidden bg-rose-500/90 text-white border-rose-400/50"></span>
      ...
      <div className="mt-2 px-1 pb-1">
        <p className="text-[9px] text-indigo-500 font-black uppercase tracking-wider mb-0.5 truncate opacity-90">
          {minifig.theme_name}
        </p>
        <h3 
          className="text-[11px] font-bold text-slate-900 line-clamp-1 leading-tight min-h-[1.2em]"
        >
          {decodedName}
        </h3>
